...
# docx_files = glob.glob("Resume&Job_Description/Original_Resumes/*.docx",recursive=True)
# pdf_files = glob.glob("Resume&Job_Description/Original_Resumes/*.pdf",recursive=True)
# rtf_files = glob.glob("Resume&Job_Description/Original_Resumes/*.rtf",recursive=True)
# text_files = glob.glob("Resume&Job_Description/Original_Resumes/*.txt",recursive=True)
files = set(doc_files)
files = list(files)

# for name in glob.glob('Resume&Job_Description/Original_Resumes/*.doc'):
    # print(name)

# files  =  glob.glob("/Users/mahika/Documents/21/dataset/*.pdf",recursive=True)
# files  =  glob.glob("Resume&Job_Description/Original_Resumes/*.pdf",recursive=True)
...
for f in files:
    info = {}
    tika.initVM()
    text =  str(parser.from_file(f)["content"]).strip()
    nlp_text = nlp(text)      
